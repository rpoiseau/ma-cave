...
**Règle — une appellation multicolore = un profil par couleur.** Quand une AOC produit plusieurs couleurs (rouge/blanc/rosé/effervescent/liquoreux), on crée **un `AppellationProfile` distinct par couleur** plutôt qu'une entrée unique : la garde, les cépages et la température de service diffèrent par couleur, et `BottleForm.vue` matche par `name`. Convention : `id` suffixé (`<id>-rouge`/`-blanc`/`-rose`/`-petillant`/`-liquoreux`) et `name` suffixé par la couleur (« Cheverny Rouge », « Cheverny Blanc », « Cheverny Rosé »). Ne jamais regrouper plusieurs couleurs sous un seul profil. (Quelques anciennes entrées mono-couleur d'AOC en réalité multicolores restent à migrer au fil de l'eau.)
